<div class="wrap email-keep">
    ...
    <div class="email-keep-nav">
        <a href="admin.php?page=email-keep%2Finbox&type=new" title="view new messages"><div <?php if ($type == 'new') {
    ?> class="active" <?php
} ?>>New <span><?php echo $counts['new']; ?></span></div></a>
        <a href="admin.php?page=email-keep%2Finbox&type=read" title="view read messages"><div <?php if ($type == 'read') {
        ?> class="active" <?php
    } ?>>Read <span><?php echo $counts['read']; ?></span></div></a>
        <a href="admin.php?page=email-keep%2Finbox&type=deleted" title="view deleted messages"><div <?php if ($type == 'deleted') {
        ?> class="active" <?php
    } ?>>Deleted <span><?php echo $counts['deleted']; ?></span></div></a>
    </div>
    <form class="email-keep-form" action="admin.php?page=email-keep%2Finbox&type=<?php echo $type; ?>" method="post">
        ...
        <div class="email-keep-actions">
            <select name="action" title="select action">
                ...
                <option value="deleted"><?php if ($type == 'deleted') {
                    echo 'Permanently ';
                } ?>Delete checked messages</option>
            </select>
